...
function hasActiveOverride(item: WorkspaceEntitlement, now = Date.now()) {
  if (item.override_value_json === null || item.override_expires_at === null) return false;
  const expiresAt = Date.parse(item.override_expires_at);
  return Number.isFinite(expiresAt) && expiresAt > now;
}

function entitlementValue(item: WorkspaceEntitlement, now = Date.now()) {
  if (hasActiveOverride(item, now)) return JSON.stringify(item.override_value_json);
