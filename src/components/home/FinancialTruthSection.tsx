      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-3">
          <span className="text-xs font-bold text-[#0E9F8E] uppercase tracking-wider bg-[#EAF8F5] px-3 py-1 rounded-full border border-[#B2E5DF]">
            {lang === 'ro' ? 'Nucleul C01 & C02' : 'C01 & C02 Core'}
          </span>
          <h2 className="text-3xl sm:text-5xl font-display font-extrabold text-[#102A43] tracking-tight">
            {lang === 'ro' ? 'Un singur adevăr financiar pentru fiecare leu' : 'One Financial Source of Truth for Every Cent'}
          </h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-14">
          ...
          <div className="card-proptech p-5 space-y-2 border-l-4 border-l-[#102A43]">
            <div className="flex items-center gap-2 text-sm font-bold text-[#102A43]">
              ...
              <span>{lang === 'ro' ? 'Partidă Dublă Verificabilă' : 'True Double-Entry'}</span>
            </div>
            <p className="text-xs text-[#52667A] leading-relaxed">
              {lang === 'ro' ? 'Plan de conturi adaptat asociațiilor de proprietari. Balanță contabilă mereu echilibrată Debit = Credit.' : 'Full general ledger with debit/credit balance validation across all fund accounts.'}
            </p>
          </div>
          ...
          <div className="card-proptech p-5 space-y-2 border-l-4 border-l-[#0E9F8E]">
            <div className="flex items-center gap-2 text-sm font-bold text-[#102A43]">
              ...
              <span>{lang === 'ro' ? 'Alocare 100% Explicabilă' : 'Explainable Math'}</span>
            </div>
            <p className="text-xs text-[#52667A] leading-relaxed">
              {lang === 'ro' ? 'Cote părți indivize (CPI), număr persoane, suprafață utilă sau consum contoare, vizibile detaliat fiecărui proprietar.' : 'Statutory CPI, surface area, resident counts, and meter consumption rules.'}
            </p>
          </div>
          ...
          <div className="card-proptech p-5 space-y-2 border-l-4 border-l-[#FF7A59]">
            <div className="flex items-center gap-2 text-sm font-bold text-[#102A43]">
              ...
              <span>{lang === 'ro' ? 'Stornare, Nu Ștergere' : 'Reversals Not Deletions'}</span>
            </div>
            <p className="text-xs text-[#52667A] leading-relaxed">
              {lang === 'ro' ? 'Erorile se corectează exclusiv prin tranzacții de stornare. Istoricul financiar este protejat de modificări retroactive.' : 'Past accounting entries are never silently deleted; corrections require explicit reversing entries.'}
            </p>
          </div>
          ...
          <div className="card-proptech p-5 space-y-2 border-l-4 border-l-[#2F80ED]">
            <div className="flex items-center gap-2 text-sm font-bold text-[#102A43]">
              ...
              <span>{lang === 'ro' ? 'Pachet Audit Cenzor' : 'Censor Audit Package'}</span>
            </div>
            <p className="text-xs text-[#52667A] leading-relaxed">
              {lang === 'ro' ? 'Raport complet de audit generat la un click: reconciliere bancară, facturi nealocate, jurnale operațiuni.' : 'One-click verification reports with bank balance match and missing document alerts.'}
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#E2E8F0]">
            <div>
              <span className="text-xs font-bold text-[#0E9F8E] uppercase tracking-wider">
                {lang === 'ro' ? 'Simulare Interactivă Notă de Plată' : 'Interactive Charge Drill-Down Demo'}
              </span>
              <h3 className="text-xl font-bold text-[#102A43] mt-1">
                {lang === 'ro' ? 'Cum se descompune o linie de întreținere (Ap. 14 — 78 mp)' : 'Deconstruction of a Monthly Statement Line (Apt. 14 — 78 sqm)'}
              </h3>
            </div>
            <div className="text-xs text-[#52667A]">
              {lang === 'ro' ? 'Apasă pe o linie pentru a vedea sursa din spate:' : 'Click a line to drill down into source calculation:'}
            </div>
          </div>
              ...
              {MOCK_CHARGE_BREAKDOWN.map((line) => (
                ...
                >
                  <div>
                    ...
                    <div className="text-xs text-[#52667A] mt-0.5 font-mono">{line.supplierInvoiceRef} · Metodă: {line.allocationMethod}</div>
                  </div>
                  <div className="text-right">
                    ...
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-[#EAF8F5] text-[#0A6E62]">
                      {line.operationalPayer === 'TENANT' ? (lang === 'ro' ? 'Chiriaș / Consum' : 'Tenant / Use') : (lang === 'ro' ? 'Proprietar' : 'Owner')}
                    </span>
                  </div>
                </button>
              ))}
              ...
              <div className="bg-white rounded-2xl border border-[#D3DCE6] p-6 space-y-5 shadow-card">
                ...
                <div className="flex items-center justify-between pb-4 border-b border-[#E2E8F0]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-[#102A43]">{lang === 'ro' ? 'Fișă Justificativă de Calcul' : 'Statutory Allocation Proof'}</span>
                  </div>
                  <span className="text-xs font-mono text-[#7B8A9A]">ID: {currentItem.id}</span>
                </div>

                <div className="space-y-3 text-xs">
                  <div className="flex justify-between p-3 rounded-lg bg-[#F6F9FC]">
                    <span className="text-[#52667A]">{lang === 'ro' ? 'Factură Furnizor Sursă:' : 'Source Supplier Invoice:'}</span>
                    <span className="font-bold text-[#102A43] font-mono">{currentItem.supplierInvoiceRef}</span>
                  </div>

                  <div className="flex justify-between p-3 rounded-lg bg-[#F6F9FC]">
                    <span className="text-[#52667A]">{lang === 'ro' ? 'Total Factură Asociație:' : 'Total Association Invoice:'}</span>
                    <span className="font-bold text-[#102A43] tabular-nums font-mono">{currentItem.totalInvoiceAmount.toFixed(2)} RON</span>
                  </div>

                  <div className="flex justify-between p-3 rounded-lg bg-[#F6F9FC]">
                    <span className="text-[#52667A]">{lang === 'ro' ? 'Cota Parte / Bază de Calcul Ap. 14:' : 'Share Ratio for Unit 14:'}</span>
                  ...
                  <div className="flex justify-between p-3 rounded-lg bg-[#EAF8F5] border border-[#B2E5DF]">
                    <span className="font-bold text-[#0A6E62]">{lang === 'ro' ? 'Sumă Datorată de Apartament:' : 'Calculated Share Due:'}</span>
