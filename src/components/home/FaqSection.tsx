...
export const FaqSection: React.FC<FaqSectionProps> = ({ lang }) => {
  ...
  const faqs = [
    {
      q: lang === 'ro' ? 'Cine deține datele asociației și ale proprietarilor?' : 'Who owns the association and owner data?',
      a: lang === 'ro' 
    ...
    {
      q: lang === 'ro' ? 'Cum funcționează migrarea din softul vechi (BlocManager, Xisoft, Excel)?' : 'How does migration work from legacy software or spreadsheets?',
      a: lang === 'ro'
    ...
    {
      q: lang === 'ro' ? 'Poate o singură persoană să aibă mai multe roluri simultan?' : 'Can one person have multiple roles simultaneously?',
      a: lang === 'ro'
    ...
    {
      q: lang === 'ro' ? 'Ce văd chiriașii în aplicație? Au acces la datele financiare ale proprietarului?' : 'What do tenants see? Do they have access to owner financials?',
      a: lang === 'ro'
    ...
    {
      q: lang === 'ro' ? 'Cum funcționează citirea indexului contoarelor prin foto OCR?' : 'How does photo OCR meter reading work?',
      a: lang === 'ro'
    ...
    {
      q: lang === 'ro' ? 'Este CLADORA disponibilă ca aplicație mobilă?' : 'Is CLADORA available as a mobile application?',
      a: lang === 'ro'
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center space-y-3">
          <span className="text-xs font-bold text-[#0E9F8E] uppercase tracking-wider bg-[#EAF8F5] px-3 py-1 rounded-full border border-[#B2E5DF]">
            {lang === 'ro' ? 'Răspunsuri la Întrebări Frecvente' : 'Frequently Asked Questions'}
          </span>
          <h2 className="text-3xl sm:text-4xl font-display font-extrabold text-[#102A43]">
            {lang === 'ro' ? 'Tot ce trebuie să știi despre CLADORA' : 'Everything You Need to Know'}
          </h2>
            ...
            return (
              ...
              >
                <button
                  ...
                  className="w-full p-5 text-left flex items-center justify-between gap-4 font-bold text-[#102A43] hover:text-[#0E9F8E] transition-colors"
                >
