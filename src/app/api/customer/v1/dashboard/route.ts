...
export async function GET(request:NextRequest){
  ...
  const [{error:ledgerError},{error:billingError}]=await Promise.all([
  ...
  return NextResponse.json({...dashboard,modules:Array.from(new Set([...modules,...(ledgerError?[]:['accounting']),...(billingError?[]:['billing'])]))},{headers:HEADERS});
}
