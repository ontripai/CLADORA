...
export async function GET(request:NextRequest){
  ...
  const {error:ledgerError}=await supabase.schema('finance').rpc('get_customer_ledger',{p_context_id:parsed.data,p_limit:1,p_offset:0});
  ...
  return NextResponse.json({...dashboard,modules:ledgerError?modules:Array.from(new Set([...modules,'accounting']))},{headers:HEADERS});
}
