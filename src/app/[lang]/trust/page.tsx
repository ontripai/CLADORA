...
export async function generateMetadata({
  ...
  return {
    title: isRo ? 'Securitate, Confidențialitate (GDPR) & Conformitate | CLADORA' : 'Security, GDPR Privacy & Compliance Architecture | CLADORA',
    description: isRo
...
export default function TrustPage({
  ...
  const securityPillars = [
    {
      title: isRo ? 'Criptare End-to-End & Protecție Date' : 'End-to-End Encryption & Tenant Isolation',
      desc: isRo
    ...
    {
      title: isRo ? 'Conformitate GDPR & Drepturile Locatarilor' : 'GDPR Compliance & Purpose Minimization',
      desc: isRo
    ...
    {
      title: isRo ? 'Jurnal de Audit Imuabil (Tamper-Evident)' : 'Immutable Tamper-Evident Audit Trails',
      desc: isRo
    ...
    {
      title: isRo ? 'Disponibilitate 99.9% & Backup Continuu' : '99.9% Uptime & Point-in-Time Recovery',
      desc: isRo
    ...
    <div className="pt-32 pb-24 space-y-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="text-center max-w-3xl mx-auto space-y-4">
        ...
        <h1 className="text-4xl sm:text-6xl font-display font-extrabold text-white tracking-tight">
          {isRo ? 'Securitate și Încredere la Nivel Enterprise' : 'Enterprise Security, Privacy & Reliability'}
        </h1>
      ...
      <div id="gdpr" className="p-8 sm:p-12 rounded-3xl bg-surface-100/90 border border-white/10 space-y-6">
        <h2 className="text-2xl font-display font-bold text-white flex items-center gap-2">
          ...
          <span>{isRo ? 'Angajamentul Nostru GDPR pentru România și Uniunea Europeană' : 'GDPR Commitment & Resident Rights'}</span>
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs sm:text-sm text-slate-300">
          <div className="space-y-2 p-4 rounded-xl glass-panel">
            <span className="font-bold text-brand-300 block">Dreptul la Portabilitate</span>
          ...
          <div className="space-y-2 p-4 rounded-xl glass-panel">
            <span className="font-bold text-brand-300 block">Minimizarea Datelor</span>
            <p className="text-slate-400 text-xs">
              {isRo 
          ...
          <div className="space-y-2 p-4 rounded-xl glass-panel">
            <span className="font-bold text-brand-300 block">Dreptul de Acces & Ștergere</span>
            <p className="text-slate-400 text-xs">
              {isRo 
