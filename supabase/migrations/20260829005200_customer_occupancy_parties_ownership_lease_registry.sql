...
begin
 if tg_table_schema='portfolio'and tg_table_name='ownerships' then
  ...
  if tg_op<>'DELETE'then
   ...
   if exists(select 1 from portfolio.ownerships o where o.id<>new.id and o.unit_id=new.unit_id and o.party_id=new.party_id and daterange(o.valid_from,coalesce(o.valid_to,'infinity'::date),'[)')&&daterange(new.valid_from,coalesce(new.valid_to,'infinity'::date),'[)'))then raise exception 'overlapping_party_ownership';end if;
...
create trigger customer_occupancy_lifecycle_integrity before insert or update or delete on occupancy.lifecycle_events for each row execute function occupancy.enforce_customer_registry_integrity();
