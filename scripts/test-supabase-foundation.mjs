...
check(server.includes("cookies } from 'next/headers'"), 'server client uses request cookies');
...
check(middleware.includes("'/:lang(ro|en|fa)/app/:path*'"), 'route matcher protects localized app');
...
check(protectedLayout.includes('redirect(`/${params.lang}/login'), 'server layout rejects unauthenticated access');
