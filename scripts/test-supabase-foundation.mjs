...
check(callback.includes('password-recovery-result?status=invalid') && callback.includes("'Cache-Control': 'no-store, private'"), 'expired recovery callbacks fail safely without caching');
check(nextConfig.includes('/forgot-password') && nextConfig.includes('/reset-password') && nextConfig.includes('sensitiveAuthHeaders'), 'all recovery pages receive explicit no-store headers');
